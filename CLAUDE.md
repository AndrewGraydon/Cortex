...
## Key Files
- **Design scope:** `docs/design/scope-v0.1.md` (primary architecture doc, v0.1.26)
...
## Current Phase
**End-to-end integration complete.** All building blocks wired together into a working assistant.
2,224 unit + integration tests passing. 8/8 NPU hardware tests passing on Pi.
Voice pipeline, web chat, multi-turn context, agent routing, real HAL services all connected.
Next: Phase 5 — IoT integration (MQTT, Home Assistant), Wyoming protocol, proactive intelligence.
...
**End-to-end flow:**
Button press → ASR → IntentRouter (regex match → tool, no LLM) or ContextAssembler
→ VLM with conversation history → streaming TTS → speaker.
Web chat: same AgentProcessor, calls NPU directly when `used_llm=True`.

## Design Decisions (DD-001 through DD-053)
Key choices: Python 3.11+, Qwen3-VL-2B unified VLM (1,771 MiB measured, ~10 tok/s, DD-051),
Kokoro-82M TTS, SenseVoice ASR, FastAPI backend, SQLite + sqlite-vec, ZeroMQ IPC,
...
- CLI: `cortex run [--mock/--no-mock]`, `cortex config`, `cortex version`

## Code Style
- Linter: ruff (target Python 3.11, 100-char line length)
- Type checking: mypy (strict mode)
- Tests: pytest with pytest-asyncio (2,224 tests passing)
...
- Scope doc version bumps on changes (currently v0.1.26)
...
- NPU memory: 8GB total, 7040 MiB usable. Model budget: 2,254 MiB (32% used, 68% headroom)
