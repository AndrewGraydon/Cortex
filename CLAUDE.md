...
## Design Decisions (DD-001 through DD-013)
Key choices: Python 3.11+, Qwen3-1.7B primary LLM, Kokoro-82M TTS, SenseVoice ASR,
FastAPI backend, SQLite + sqlite-vec, ZeroMQ IPC, bubblewrap sandbox, systemd services.
LCD display adapted from PiSugar whisplay-ai-chatbot (Pillow + cairosvg + SPI).
Web UI framework deferred to Phase 3.
...
- Scope doc version bumps on changes (currently v0.1.2)
...
- NPU memory: 8GB total, ~7GB usable. Current model budget: ~4.25GB
