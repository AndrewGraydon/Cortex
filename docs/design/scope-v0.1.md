...
## System Design Scope Document v0.1.23
...
- **NPU memory (8GB)** limits model sizes. Primary VLM: Qwen3-VL-2B (1,771 MiB measured, DD-051). ASR/TTS models share this memory budget — total 2,254 MiB (32% of usable 7,040 MiB, 68% headroom).
...
```json
{
  ...
  "components": {
    "npu": {"status": "healthy", "temp_c": 62, "cmm_used_mb": 4950, "cmm_total_mb": 7040},
...
*Phase 0 measured values on M.2 + Pi 5 (AXCL v3.6.5, PCIe host via pyaxengine where noted):*
...
t=6.5s  Kokoro synthesizes sentence 2 (overlapped with playback of sentence 1)
t=8.5s  Third sentence complete → synthesize → play seamlessly
```
...
|---|---|---|---|
| Mic input (ASR) | S16_LE mono PCM | 16,000 Hz | WM8960 codec via ALSA `default` device (plug→dsnoop, DD-049) |
...
| DD-011 | Kokoro-82M as TTS engine (replacing MeloTTS) | 2026-02-27 | 2x faster on NPU (RTF 0.067 native, RTF 0.115 measured via Python/pyaxengine on Pi host), #1 HuggingFace TTS Arena quality, 54 voices, 232 MiB CMM (measured), actively maintained, already proven on LLM-8850. No C++ AXCL aarch64 binary — Python path required from Pi host. |
...
| DD-052 | KV cache compaction via Attention Matching | 2026-03-09 | **Phase 5 research — MIT technique for 50x KV cache compression.** Three-step process: (1) Reference Queries — synthetic prompts simulate interactions, 90% compute reduction vs exhaustive probing; (2) Key Selection — rank tokens by aggregated attention scores, retain ~2% of originals; (3) Value Fitting — OLS regression approximates discarded values in linear time, 2-3x better fidelity than eviction. Results: 1GB → ~20MB per 8K-token document, 0% accuracy loss on long-document QA, 4→200+ concurrent sessions on 40GB GPU. Tested on AIME math: model solved problems after 6 consecutive mid-thought 50% compressions. **Cortex relevance:** Not directly actionable — requires runtime-level support from AXERA (AXCL/Pulsar2). Our 2,047-token limit is a compiled axmodel constraint, not purely KV memory. However, if AXERA adopts this, could enable 4K-8K token models within same 7GB budget, significantly reducing context assembly pressure. Track as future NPU ecosystem enhancement. Source: VentureBeat, March 2026. |
| DD-053 | LLM-driven memory consolidation (no vector DB) | 2026-03-09 | **Phase 5 design input — Google's "Always On Memory Agent" architecture.** Eliminates embedding pipeline entirely: "No vector database. No embeddings. Just an LLM that reads, thinks, and writes structured memory." Agent runs continuously, ingests multimodal input, stores structured memories in SQLite, performs scheduled LLM-driven consolidation every 30 minutes. Built on Google ADK + Gemini Flash-Lite. **Cortex hybrid approach for Phase 5:** Keep embedding-based retrieval (fast, ~1ms numpy cosine) for real-time search, but add periodic LLM-driven "dreaming" consolidation during idle time: (1) Review recent episodic events via proactive engine's idle loop; (2) LLM synthesizes higher-level insights from raw event clusters ("user checks clock every morning at 8am" rather than 10 raw records); (3) Write consolidated facts back to long-term memory with embeddings. This extends the rule-based `EpisodicMemoryStore.get_routine_patterns()` (Phase 4) with semantic pattern extraction. Aligns with DD-038 (proactive intelligence) Phase 5 LLM-based correlation. Their pure-LLM approach unsuitable for edge/NPU (every memory op = inference pass, ~2-10s on our hardware; consolidation would block voice pipeline). Source: VentureBeat/Google CloudPlatform, March 2026. |

*Document version: 0.1.26 — Session 22: Phase 4 complete (1,524 tests). DD-052 (KV cache compaction) and DD-053 (LLM-driven memory consolidation) added as Phase 5 research inputs.*
*Status: DRAFT*
